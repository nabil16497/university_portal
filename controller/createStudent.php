...
require_once '../model.php';
	...
	if(empty($fnameerror)&& empty($lnameerror) && empty($doberror) && empty($gendererror) && empty($contacterror)&& empty($emailerror) && empty($nationalityerror) && empty($addresserror) && empty($blooderror)&& empty($depterror) && empty($programerror) && empty($pperror))
 {
 	...
 	if (addStudent($data)) {
  		echo "<script>alert('Student Sucessfully Registered.'); window.location.href='../studentreg.php';</script>";
  ...
  $errorm = " | ".$fnameerror."  |  ".$lnameerror."  |  ".$doberror."  |  ".$gendererror."  |  ".$contacterror."  |  ".$emailerror."  |  ".$nationalityerror."  |  ".$addresserror."  |  ".$blooderror."  |  ".$depterror."  |  ".$programerror."  |  "." | ".$pperror." | ";
    echo "<script>alert('Error-".$errorm."'); window.location.href='../studentreg.php';</script>"; 
  		...
  		echo "<script>alert('Access not allowed.'); window.location.href='../studentreg.php';</script>";
